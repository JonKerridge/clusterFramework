...
class HostRun {
  ...
  Class emitClass
  Class collectClass
  Class dataLoadClass = null
  String fileBasename
  int portNumber = 56789  //port number used in TCPIP communications

  // in version 2 the String name options were removed
 ...
 */
  ...
  // further versions added in V2 to support shared data in worker nodes
 ...
 * @param emitClass the class containing the object to be processed
 * @param collectClass the class used to collect the resulting output
 * @param dataLoadClass the class used for worker shared data object
 */
  HostRun(String structureFile,
          Class emitClass,
          Class collectClass,
    ...
    this.emitClass = emitClass
    this.collectClass = collectClass
    this.dataLoadClass = dataLoadClass
    nature = "Net"
  }
/**
 * A class used to run the host node of a cluster
...
//send classes used to emit, collect and sharedData (v2) objects to each nodeLoader
    ClassDefinitions classDefinitions = new ClassDefinitions(emitClass: emitClass,
        collectClass: collectClass, sharedDataClass: dataLoadClass, version: version)
  ...
  static void main(String[] args) {
    switch (args.size()){
      ...
      case 4:
        ...
        break
      case 5:
        new HostRun(args[0], args[1], args[2], args[3], args[4]).invoke()
        break
      default:
        println "HostRun: wrong number of arguments ${args} supplied" +
            ...
            "\n\targs[3] - optional - class used for shared data in worker nodes"+
            "\n\targs[3 or 4] - optional - if specified has value 'Local'"
    }
  }
